...
public class ProcessesPanelPresenter extends BasePresenter implements ProcessesPanelView.ActionDelegate,
    ...
    /**
     * Determines the agent in injected in the specified machine.
     *
     * @param machineName
     *          machine name
     * @param agent
    ...
    private boolean hasAgent(String machineName, String agent) {
        ...
        for (Environment environment : workspace.getConfig().getEnvironments().values()) {
    ...
    private ProcessTreeNode addMachineNode(MachineEntity machine) {
        ...
        machineNode.setHasTerminalAgent(hasAgent(machine.getDisplayName(), TERMINAL_AGENT));
        machineNode.setHasSSHAgent(hasAgent(machine.getDisplayName(), SSH_AGENT));
